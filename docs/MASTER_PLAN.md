...
| # | Идея | Описание |
|---|------|----------|
| 1 | **Стартовый набор ниш/ключей** | Запрос к DataForSeo (или аналогу) по списку ниш/микрониш (из справочника или из офферов) → топ ключей по объёму/конкуренции. Внешняя аналитика подсказывает, в каких нишах есть спрос, а не только ключи по одной выбранной нише. |
| 2 | **Приоритет офферов при нулевых данных** | Сейчас «лучший оффер» только по нашему ROI. Добавить внешний сигнал для холодного старта: данные из API партнёрки (если есть) или эвристики по эпику/конверту из описания оффера, гео, типу креатива. При отсутствии конверсий система могла бы выбирать оффер по внешней аналитике. |
| 3 | **Авто-создание первой кампании из офферов** | Сценарий: офферы из партнёрки → по `offer-country-recommendations` выбрать гео → по DataForSeo сгенерировать ключи для этих гео/ниш → создать одну кампанию + N дорвеев. Внешняя аналитика участвует в выборе ниши и первого набора ключей. |
