...
def upgrade():
    conn = op.get_bind()
    if not _table_exists(conn, "visitor_events"):
        op.create_table(
            "visitor_events",
    ...
    # Idempotent index creation (e.g. after partial run)
    inspector = reflection.Inspector.from_engine(conn)
    existing_visitor_events_indexes = {i["name"] for i in inspector.get_indexes("visitor_events")}
    for idx, table, cols in [
        ("ix_visitor_events_doorway_id", "visitor_events", ["doorway_id"]),
        ("ix_visitor_events_campaign_id", "visitor_events", ["campaign_id"]),
        ("ix_visitor_events_created_at", "visitor_events", ["created_at"]),
    ]:
        if idx not in existing_visitor_events_indexes:
            op.create_index(idx, table, cols)
