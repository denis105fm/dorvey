...
1. **Серверы** — добавить хотя бы один сервер (хост, SSH-ключ или пароль, путь). Без сервера нельзя привязать домен.
2. **Домены** — добавить домен(ы), привязать к серверу. Можно один домен на 10+ дорвеев (разные пути).
3. **Настройки** — минимум для старта: **OpenAI API ключ** (если нужна AI-генерация контента). GSC, Bing, DataForSeo можно подключить позже (для индексации и подсказки ключей).
4. **Кампании** — создать кампанию: название, Affiliate URL, язык/локаль/регион. Регион кампании должен совпадать с целевой страной в поиске (см. ниже про GSC).
5. **Офферы** (по желанию) — добавить офферы с гео и device (mobile/desktop). Если один оффер — достаточно Affiliate URL у кампании.
6. **Дорвеи** — сгенерировать по ключам (кампания + домен), задеплоить. После деплоя при необходимости добавить свойство в GSC и отправить URL.

**Целевая страна в поиске (гео-таргетинг):**  
Сервер может быть в любой стране (Европа, США — не важно). Для того чтобы поисковик показывал сайт пользователям нужной страны: в **Google Search Console** для свойства домена укажите целевую страну (International Targeting). Регион кампании (поле **Регион**) держите в том же значении (например `US` для США, `RU` для России) — он используется в контенте, DataForSeo и офферах.
